...
import os
import sys
os.system("ln -s /src/AnimateDiff /AnimateDiff")
...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        os.system("rm -rf /AnimateDiff/models/StableDiffusion")
    ...
    def predict(
        ...
        steps: int = Input(description="Number of inference steps", ge=1, le=100, default=6),
        guidance_scale: float = Input(description="guidance scale", ge=1, le=10, default=1),
        ...
        self.scheduler = EulerDiscreteScheduler(timestep_spacing='leading', steps_offset=1,**OmegaConf.to_container(inference_config.noise_scheduler_kwargs))
        
        self.pipeline = AnimationPipeline(
            vae=self.vae, text_encoder=self.text_encoder, tokenizer=self.tokenizer, unet=self.unet,
            text_encoder_2 = self.text_encoder_two, tokenizer_2=self.tokenizer_two, scheduler=self.scheduler,
        )
        self.pipeline.load_lora_weights("latent-consistency/lcm-lora-sdxl", cache_dir="lcm-cache")
